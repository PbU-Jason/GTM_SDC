...
void open_all_file(char *InputFilePath, char *OutFilePath) {
    ...
    switch (decode_mode) {
        ...
        case 2:
            ...
            fputs(tmtc_header_new_ouput, raw_outfile);
            free(raw_outpath);
            ...
            fputs(tmtc_header_new_ouput, tmtc_master_outfile);
            ...
            fputs(tmtc_header_new_ouput, tmtc_slave_outfile);
...
void parse_tmtc_packet_new_output(unsigned char *Target) {
    int i; // for citiroc hit
    int j; // for GTM module
    ...
    *(Target + 15) = *(Target + 15) & 0x7F; // mask GTM ID
    ...
    // citiroc temp (output is int16_t, but use unsigned char [2] is easier to process!)
    // make from 1 byte, thus, will manually considered the big2little_endian! // see fine counter

    // log_message("Target + 22 = %X", *(Target + 22));
    if ((*(Target + 22) & 0x80) == 0x80) {
        tmtc_buffer->citiroc1_temp1[0] = 0xFF;
    }
    else {
        tmtc_buffer->citiroc1_temp1[0] = 0x00;
    }
    memcpy(&(tmtc_buffer->citiroc1_temp1[1]), Target + 22, 1);
    // log_message("tmtc_buffer->citiroc1_temp1 = %X", tmtc_buffer->citiroc1_temp1[0]);
    // log_message("tmtc_buffer->citiroc1_temp1 = %X", tmtc_buffer->citiroc1_temp1[1]);

    if ((*(Target + 23) & 0x80) == 0x80) {
        tmtc_buffer->citiroc1_temp2[0] = 0xFF;
    }
    else {
        tmtc_buffer->citiroc1_temp2[0] = 0x00;
    }
    memcpy(&(tmtc_buffer->citiroc1_temp2[1]), Target + 23, 1);

    if ((*(Target + 24) & 0x80) == 0x80) {
        tmtc_buffer->citiroc2_temp1[0] = 0xFF;
    }
    else {
        tmtc_buffer->citiroc2_temp1[0] = 0x00;
    }
    memcpy(&(tmtc_buffer->citiroc2_temp1[1]), Target + 24, 1);

    if ((*(Target + 25) & 0x80) == 0x80) {
        tmtc_buffer->citiroc2_temp2[0] = 0xFF;
    }
    else {
        tmtc_buffer->citiroc2_temp2[0] = 0x00;
    }
    memcpy(&(tmtc_buffer->citiroc2_temp2[1]), Target + 25, 1);

    // citiroc livetime
    memcpy(&(tmtc_buffer->citiroc1_livetime_busy), Target + 26, 3); // see fine counter
    // big2little_endian(&(tmtc_buffer->citiroc1_livetime), 3);
    memcpy(&(tmtc_buffer->citiroc2_livetime_busy), Target + 29, 3); // see fine counter
    // big2little_endian(&(tmtc_buffer->citiroc2_livetime), 3);
    // citiroc hit
    for (i = 0; i < 32; ++i) {
        memcpy(&(tmtc_buffer->citiroc1_hit[i]), Target + 32 + i, 1);
    ...
    // citiroc livetime
    memcpy(&(tmtc_buffer->citiroc1_livetime_buffer_busy), Target + 119, 3); // see fine counter
    memcpy(&(tmtc_buffer->citiroc2_livetime_buffer_busy), Target + 122, 3); // see fine counter
...
void write_tmtc_buffer_new_output(void) { 
    ...
    int16_t citiroc1_temp1 = 0;
    int16_t citiroc1_temp2 = 0;
    int16_t citiroc2_temp1 = 0;
    int16_t citiroc2_temp2 = 0;
    ...
    citiroc1_temp1 = ( tmtc_buffer->citiroc1_temp1[0] << 8) |  tmtc_buffer->citiroc1_temp1[1];
    citiroc1_temp2 = ( tmtc_buffer->citiroc1_temp2[0] << 8) |  tmtc_buffer->citiroc1_temp2[1];
    citiroc2_temp1 = ( tmtc_buffer->citiroc2_temp1[0] << 8) |  tmtc_buffer->citiroc2_temp1[1];
    citiroc2_temp2 = ( tmtc_buffer->citiroc2_temp2[0] << 8) |  tmtc_buffer->citiroc2_temp2[1];

    fprintf(raw_outfile, "%X%X", tmtc_buffer->head[0], tmtc_buffer->head[1]); // head
    fprintf(raw_outfile, ";%u;%u;%u;%u;%u;%u;%u;%u;%u;%u;%i;%u;%i;%i;%i;%i;%i;%i;%i;%i;%i", tmtc_buffer->gtm_id, tmtc_buffer->packet_counter, tmtc_buffer->data_length_msb, tmtc_buffer->data_length, time_buffer->year, time_buffer->day, time_buffer->hour, time_buffer->minute, time_buffer->sec, time_buffer->sub_sec, tmtc_buffer->gtm_id_in_pps_counter, tmtc_buffer->pps_counter, fine_counter, tmtc_buffer->board_temp1, tmtc_buffer->board_temp2, citiroc1_temp1, citiroc1_temp2, citiroc2_temp1, citiroc2_temp2, citiroc1_livetime_busy, citiroc2_livetime_busy);
    for (i = 0; i < 32; ++i) {
        fprintf(raw_outfile, ";%u", tmtc_buffer->citiroc1_hit[i]);
    }
    for (i = 0; i < 32; ++i) {
        fprintf(raw_outfile, ";%u", tmtc_buffer->citiroc2_hit[i]);
    }
    fprintf(raw_outfile, ";%u;%u;%u;%u;%u;%X;%X;%X;%X;%X;%X;%u;%u;%u", tmtc_buffer->citiroc1_trigger, tmtc_buffer->citiroc2_trigger, tmtc_buffer->counter_period, tmtc_buffer->hv_dac1, tmtc_buffer->hv_dac2, tmtc_buffer->spw_a_error_count, tmtc_buffer->spw_a_last_receive, tmtc_buffer->spw_b_error_count, tmtc_buffer->spw_b_last_receive, tmtc_buffer->spw_a_status, tmtc_buffer->spw_b_status, tmtc_buffer->recv_checksum, tmtc_buffer->calc_checksum, tmtc_buffer->recv_num);
    for (i = 0; i < 5; ++i) {
        fprintf(raw_outfile, ";%u", tmtc_buffer->tmtc_empty[i]);
    }
    fprintf(raw_outfile, ";%i;%i;%u", citiroc1_livetime_buffer_busy, citiroc2_livetime_buffer_busy, tmtc_buffer->checksum);
    fprintf(raw_outfile, ";%X%X\n", tmtc_buffer->tail[0], tmtc_buffer->tail[1]); // tail
}
...
void write_tmtc_buffer_master(void) {
    ...
    fprintf(tmtc_master_outfile, ";%u;%u;%u;%u;%u;%u;%u;%u;%u;%u;%i;%u;%i;%i;%i;%i;%i;%i;%i;%i;%i", tmtc_buffer->gtm_id, tmtc_buffer->packet_counter, tmtc_buffer->data_length_msb, tmtc_buffer->data_length, time_buffer->year, time_buffer->day, time_buffer->hour, time_buffer->minute, time_buffer->sec, time_buffer->sub_sec, tmtc_buffer->gtm_id_in_pps_counter, tmtc_buffer->pps_counter, fine_counter, tmtc_buffer->board_temp1, tmtc_buffer->board_temp2, citiroc1_temp1, citiroc1_temp2, citiroc2_temp1, citiroc2_temp2, citiroc1_livetime_busy, citiroc2_livetime_busy);
    for (i = 0; i < 32; ++i) {
    ...
    fprintf(tmtc_master_outfile, ";%u;%u;%u;%u;%u;%X;%X;%X;%X;%X;%X;%u;%u;%u", tmtc_buffer->citiroc1_trigger, tmtc_buffer->citiroc2_trigger, tmtc_buffer->counter_period, tmtc_buffer->hv_dac1, tmtc_buffer->hv_dac2, tmtc_buffer->spw_a_error_count, tmtc_buffer->spw_a_last_receive, tmtc_buffer->spw_b_error_count, tmtc_buffer->spw_b_last_receive, tmtc_buffer->spw_a_status, tmtc_buffer->spw_b_status, tmtc_buffer->recv_checksum, tmtc_buffer->calc_checksum, tmtc_buffer->recv_num);
    for (i = 0; i < 5; ++i) {
...
void write_tmtc_buffer_slave(void) {
    ...
    fprintf(tmtc_slave_outfile, ";%u;%u;%u;%u;%u;%u;%u;%u;%u;%u;%i;%u;%i;%i;%i;%i;%i;%i;%i;%i;%i", tmtc_buffer->gtm_id, tmtc_buffer->packet_counter, tmtc_buffer->data_length_msb, tmtc_buffer->data_length, time_buffer->year, time_buffer->day, time_buffer->hour, time_buffer->minute, time_buffer->sec, time_buffer->sub_sec, tmtc_buffer->gtm_id_in_pps_counter, tmtc_buffer->pps_counter, fine_counter, tmtc_buffer->board_temp1, tmtc_buffer->board_temp2, citiroc1_temp1, citiroc1_temp2, citiroc2_temp1, citiroc2_temp2, citiroc1_livetime_busy, citiroc2_livetime_busy);
    for (i = 0; i < 32; ++i) {
    ...
    fprintf(tmtc_slave_outfile, ";%u;%u;%u;%u;%u;%X;%X;%X;%X;%X;%X;%u;%u;%u", tmtc_buffer->citiroc1_trigger, tmtc_buffer->citiroc2_trigger, tmtc_buffer->counter_period, tmtc_buffer->hv_dac1, tmtc_buffer->hv_dac2, tmtc_buffer->spw_a_error_count, tmtc_buffer->spw_a_last_receive, tmtc_buffer->spw_b_error_count, tmtc_buffer->spw_b_last_receive, tmtc_buffer->spw_a_status, tmtc_buffer->spw_b_status, tmtc_buffer->recv_checksum, tmtc_buffer->calc_checksum, tmtc_buffer->recv_num);
    for (i = 0; i < 5; ++i) {
