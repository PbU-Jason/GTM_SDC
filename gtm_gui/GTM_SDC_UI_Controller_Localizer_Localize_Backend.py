...
def localize_grb(trigger_info):
    
    ### main code ###
    table_name = ['middle','hard','soft']
    table1 = np.array(pd.read_csv("./big_typical1_response.csv")) #file name of middle energy big table
    table2 = np.array(pd.read_csv("./big_typical2_response.csv")) #file name of hard energy big table
    table3 = np.array(pd.read_csv("./big_typical3_response.csv")) #file name of soft energy big table

    trigger_data = pd.read_pickle(trigger_info)
    ...
    print("Location (Ra & Dec): " + str((location_RA,location_Dec)))

    #================== position of earth and sun and milkyway =================
    # earth horizon
    print("Trigger UTC: " + trigger_data[list(trigger_data.keys())[0]][0])
    ...
    for ii in range(len(table)):
        q_table = Quaternion(0, -1*table[ii,0], table[ii,1], -1*table[ii,2])
        q_table_sky = (q_sc*q_table*q_sc.inverse).conjugate
        RA_, Dec_ = xyz2RD(q_table_sky[1],q_table_sky[2],q_table_sky[3])
        RA = np.append(RA,RA_)
        Dec = np.append(Dec,Dec_)

    phi, theta = V2AZ(table[:,0],table[:,1],table[:,2])
    ...
    plt.grid(True)
    plt.savefig(f"../level_2/{os.path.basename(trigger_info)}_AllSykMap.png", dpi=500)

    print('T50 (s):', trigger_data[list(trigger_data.keys())[6]][0])
    print('T90 (s):', trigger_data[list(trigger_data.keys())[7]][0])
    ...
    if location_Dec>70 or location_Dec<-70:
        ...
        ax2 = plt.subplot(111)
    cs2 = ax2.tricontour(RA, Dec, np.array(Chi_square, dtype = 'float'), levels = [2.3, 4.6, 9.6]+color_map_min, colors = 'purple', linewidths=3)
    plt.scatter(location_RA, location_Dec, marker='x', c ='white', s=100)
    ...
    plt.grid(True)
    plt.savefig(f"../level_2/{os.path.basename(trigger_info)}_ZoomInSykMap.png", dpi=300)
