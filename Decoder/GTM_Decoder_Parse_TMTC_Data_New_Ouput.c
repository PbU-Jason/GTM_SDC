...
void parse_tmtc_data_new_output(void) {
    ...
    while (1) {
        ...
        for (location = 0; location < actual_binary_buffer_size; location++) {
            ...
            if (tmtc_data_buffer_counter == 2) { // tmtc header
                if (!is_tmtc_header(tmtc_data_buffer)) { // is not tmtc header
                    tmtc_data_buffer[0] = tmtc_data_buffer[1];
                    tmtc_data_buffer_counter = 1;
                }
            }
            if (tmtc_data_buffer_counter == TMTC_DATA_SIZE) { // all tmtc data is loaded into buffer
                if (!is_tmtc_tail(tmtc_data_buffer + TMTC_DATA_SIZE - 2)) { // is not tmtc tail
        ...
        actual_binary_buffer_size = read_from_file(binary_buffer, bin_infile, max_binary_buffer_size); // not yet clear!!!
    }
