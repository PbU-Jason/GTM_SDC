        ...
        case 1:
            ...
            if (ftell(raw_output_file) == 0) {
                ...
                if (in_space_flag == 1) { // in space
                    fputs(tmtc_csv_column_name_all_in_space, raw_output_file);
                }
                else if (in_space_flag == 2) { // on ground
                    fputs(tmtc_csv_column_name_all_on_ground, raw_output_file);
                }
                else {
            ...
            if (ftell(tmtc_master_output_file) == 0) {
                ...
                // separate cases by in_space_flag
                if (in_space_flag == 1) { // in space
                    fputs(tmtc_csv_column_name_master_in_space, tmtc_master_output_file);
                }
                else if (in_space_flag == 2) { // on ground
                    fputs(tmtc_csv_column_name_master_on_ground, tmtc_master_output_file);
                }
                else {
                    log_error("Unknown in space flag!");
                }
            ...
            if (ftell(tmtc_slave_output_file) == 0) {
                ...
                // separate cases by in_space_flag
                if (in_space_flag == 1) { // in space
                    fputs(tmtc_csv_column_name_slave_in_space, tmtc_slave_output_file);
                }
                else if (in_space_flag == 2) { // on ground
                    fputs(tmtc_csv_column_name_slave_on_ground, tmtc_slave_output_file);
                }
                else {
                    log_error("Unknown in space flag!");
                }
...
void parse_tmtc_packet(unsigned char *target) {
    int tmtc_16_byte_shift;

    // separate cases by in_space_flag
    if (in_space_flag == 1) { // in space
        tmtc_16_byte_shift = TMTC_PACKET_HEADER_SIZE+TMTC_DATA_FIELD_HEADER_SIZE;
...
int is_science_icd_spacewire_rmap_head(unsigned char *target) {
    unsigned char ref[SPACEWIRE_RMAP_HEAD_SIZE] = {0xFE, 0x01, 0x60, 0x00, 0xFE, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x56, 0x17};

    if (!memcmp(ref, target, SPACEWIRE_RMAP_HEAD_SIZE)) {
        return 1;
    }
...
int parse_science_packet(unsigned char *target) {
    ...
    int find_weird_counter_temp = 0;
    int find_weird_counter = 0;
        ...
        if (science_buffer->gtm_id == 0) { // master case
            ...
            if (!stop_find_sync_data_header_master_flag) { // look for sync data header
                ...
                if (have_complete_sync_data_master_flag) {
                    find_weird_counter_temp = parse_science_event_data(science_data_pointer+(i*3));
                    find_weird_counter+=find_weird_counter_temp;
                }
            }
                ...
                if (science_sync_master_buffer_counter == 45) {
                    ...
                    if (!is_science_sync_tail(science_sync_master_buffer+42)) { // is not sync tail
                        ...
                        for (size_t j = 0; j < 45/3; j++) {
                            find_weird_counter_temp = parse_science_event_data(science_sync_master_buffer+(j*3));
                            find_weird_counter+=find_weird_counter_temp;
                        }
            ...
            if (!stop_find_sync_data_header_slave_flag) { // look for sync data header
                ...
                if (science_sync_slave_buffer_counter == 45) {
                    ...
                    if (!is_science_sync_tail(science_sync_slave_buffer+42)) { // is not sync tail
                        ...
                        for (size_t j = 0; j < 45/3; j++) {
                            find_weird_counter_temp = parse_science_event_data(science_sync_slave_buffer+(j*3));
                            find_weird_counter+=find_weird_counter_temp;
                        }
    ...
    // if have any find_weird_counter, report a flag
    if (find_weird_counter != 0) {
        return 1;
    }
    else {
        return 0;
