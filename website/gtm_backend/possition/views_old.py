...
def now_lon_lat(request):
    ...
    times, orbit, is_sunlight, minutes = calculate_orbit_eclipse(tle, (2023, 10, 20, 0), 720)
    ...
    # fig.write_html('mission_timeline_2d.html')
    ...
    # fig.write_html('mission_timeline_3d.html')
    ...
    return JsonResponse({'try': f'{np.random.randint(5)}', 
    'lon': f'{np.around(orbit[0,0], 3)}', 'lat': f'{np.around(orbit[0,1], 3)}'})
...
def calculate_orbit_eclipse(tle, utc, mins):
    ...
    minutes = np.arange(0, mins, 0.5)
    times = ts.utc(utc[0], utc[1], utc[2], utc[3], minutes)
    ...
    data  = load(os.path.join(settings.BASE_DIR, 'possition/data/de421.bsp'))
    Earth = data['earth']
    Sun   = data['sun']
    Sat   = Earth + satellite
    ...
    return times, orbit, is_sunlight, minutes
