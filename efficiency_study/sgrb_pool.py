    ...
    for is_shift in range(2):
        for bin_size in bin_size_list:
            ...
            for sensor_data_index in range(len(sensor_data)):
                ...
                avg = np.mean(hist)
                threshold = poisson.ppf(1-0.0001/(real_total_duration/bin_size), avg) + 1 # reasonable one
                # threshold = poisson.ppf(1-0.001/(real_total_duration/bin_size), avg) # too bad
                # threshold = poisson.ppf(1-0.0001/(real_total_duration/bin_size), avg) # not good enough
